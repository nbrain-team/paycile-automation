// FIXED: CFO Funnel Template WITHOUT SMS - Properly creates Node and Edge records
// Run: node scripts/reseed_cfo_no_sms_fixed.js

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

const TEMPLATE_ID = 'cmk4g9s1b0000cq0xsa507s23'; // Keep same ID to update existing template
...
const cfoInsuranceFunnelNoSMS = {
  ...
  nodes: [
    ...
    // HIGH ENGAGEMENT PATH (Nodes 13-20) - NO SMS
    ...
    // DAY 3 - RE-ENGAGEMENT (Nodes 31-40) - NO SMS
    ...
    // DAY 7 - FINAL PUSH (Nodes 51-60) - NO SMS
    ...
    // POSITIVE RESPONSE HANDLER (Nodes 61-70) - NO SMS
    ...
    // NO-SHOW PATH (Nodes 81-85) - NO SMS
  ...
  edges: [
    ...
    // High engagement path (opened + clicked) - NO SMS
    { from: 'N008', to: 'N013' },
    { from: 'N013', to: 'N014' },
    { from: 'N014', to: 'N015' },
    ...
    // Positive response handler - NO SMS
    { from: 'N061', to: 'N062' },
    { from: 'N062', to: 'N063' },
    { from: 'N063', to: 'N064' },
    { from: 'N064', to: 'N065' },
    ...
    { from: 'N082', to: 'N084' },
    { from: 'N084', to: 'N085' },
    { from: 'N085', to: 'N068', condition: { demo_rescheduled: true } },
    ...
    { from: 'N087', to: 'N088' },
    { from: 'N088', to: 'N089' },
    { from: 'N089', to: 'N004', condition: { email_opened: true } },
    { from: 'N089', to: 'N090', condition: { email_opened: false } },
  ]
};

// =============================================================================
// SEED FUNCTION - FIXED TO USE PROPER RELATIONS
// =============================================================================

async function seed() {
  console.log('🚀 Re-seeding CFO Funnel (No SMS) with FIXED structure...\n');
  ...
  try {
    ...
    // 2. Delete existing template and its nodes/edges
    console.log('🗑️  Deleting existing template (if exists)...');
    ...
    if (existing) {
      console.log(`   Found existing template with ${existing.nodes.length} nodes, ${existing.edges.length} edges`);
      // Delete nodes and edges first (cascade should handle this, but being explicit)
      await prisma.node.deleteMany({ where: { templateId: TEMPLATE_ID } });
      await prisma.edge.deleteMany({ where: { templateId: TEMPLATE_ID } });
      await prisma.template.delete({ where: { id: TEMPLATE_ID } });
      console.log('   ✅ Deleted\n');
    }

    // 3. Create funnel template with proper Node and Edge records
    console.log('🎯 Creating CFO-Funnel-No-SMS with proper structure...');
    ...
    console.log(`   - ${funnel.nodes.length} nodes`);
    console.log(`   - ${funnel.edges.length} edges\n`);

    // 4. Verify content node connections
    console.log('🔍 Verifying content node connections...\n');
    const contentNodes = funnel.nodes.filter(n => 
    ...
    for (const node of contentNodes) {
      if (node.config.template_id) {
        const ct = await prisma.contentTemplate.findUnique({
          where: { id: node.config.template_id }
        });
        if (ct) {
          console.log(`   ✅ ${node.key}: ${node.config.template_id} → "${ct.name}"`);
        } else {
          console.log(`   ❌ ${node.key}: template_id "${node.config.template_id}" NOT FOUND`);
        }
      }
    }
    ...
    console.log(`   - Content Templates: ${cfoInsuranceContentNoSMS.length}`);
    console.log(`\n✅ Template is now properly connected and ready to use!`);
    console.log(`   View at: https://paycile-automation.onrender.com/templates/${TEMPLATE_ID}`);
