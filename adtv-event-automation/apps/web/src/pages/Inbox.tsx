...
import { apiInbox, apiSms, apiEmail, apiCampaigns, apiContacts, apiAI } from '@lib/api';
...
type Activity = {
  id: string;
  contact: { id: string; name: string; automationCheckedOut?: boolean; automationPausedNodeKey?: string };
  channel: Channel;
  direction: Direction;
  subject?: string;
  body: string;
...
export function Inbox() {
        ...
        convos.forEach((c) => {
          (c.messages || []).forEach((m: any) =>
            mapped.push({
              ...
              contact: { 
                id: c.contactId, 
                name: c.contact?.name || 'Contact',
                automationCheckedOut: c.contact?.automationCheckedOut,
                automationPausedNodeKey: c.contact?.automationPausedNodeKey
              },
  ...
  const handleCheckIn = async () => {
    if (!selected) return;
    try {
      await apiContacts.checkin(selected.contact.id, true);
      addToast({ title: 'Checked In', description: `${selected.contact.name} has been checked back into automation`, variant: 'success' });
      // Refresh the contact data
      const convos = await apiInbox.conversations();
      const mapped: Activity[] = [];
      ...
      addToast({ title: 'Error', description: 'Failed to check in contact', variant: 'error' });
    }
  };
  ...
  const handleGenerateAIResponse = async () => {
    ...
    try {
      ...
      const incomingMessage = {
        ...
        fromEmail: 'unknown@example.com', // Will be enhanced with real contact data
        fromCompany: 'Unknown',
        subject: selected.subject || '',
        body: selected.body,
        timestamp: selected.time,
        thread: history.slice(0, -1).map(a => ({
          id: a.id,
          from: a.direction === 'inbound' ? selected.contact.name : 'Katie',
          fromEmail: '',
          subject: a.subject || '',
          body: a.body,
          timestamp: a.time,
      ...
      // Attempt new Newbury-trained AI generator
      const result = await apiAI.generateInboxResponse(
        incomingMessage,
        { name: selected.contact.name, company: 'Unknown' },
        undefined, // campaign context - could be enhanced
        undefined  // BDR notes
      );

      if (result.success && result.response) {
        const generatedBody = selected.channel === 'email' && result.response.subject 
          ? `Subject: ${result.response.subject}\n\n${result.response.body}`
          : result.response.body;
        
        setReply(generatedBody);
        addToast({ 
          title: 'AI Response Generated (Newbury Voice)', 
          description: `${result.response.confidence} confidence · ${result.response.rationale}`, 
          variant: 'success' 
        });
      }
    } catch (e: any) {
      // Fallback to original AI generator
      try {
        const history = activities
          .filter((a) => a.contact.id === selected.contact.id)
          .sort((a, b) => dayjs(a.time).valueOf() - dayjs(b.time).valueOf())
          .map((a) => ({
            direction: a.direction === 'inbound' ? 'in' as const : 'out' as const,
            text: a.body,
            time: a.time,
          }));
        const fallbackResult = await apiAI.generateResponse(selected.contact.id, history);
        if (fallbackResult.ok && fallbackResult.response) {
          setReply(fallbackResult.response);
          addToast({ title: 'AI Response Generated (Fallback)', description: 'Review and edit before sending', variant: 'success' });
        }
      } catch (e2) {
        addToast({ title: 'Error', description: 'Failed to generate AI response', variant: 'error' });
      }
    } finally {
            ...
            <div className="card">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {selected.contact.automationCheckedOut ? (
                    <div className="flex items-center gap-2">
                      <span className="badge-gray text-xs">🚫 Checked Out</span>
                      <button className="btn-outline btn-sm" onClick={handleCheckIn}>
                        ✓ Check In to Automation
                      </button>
                    </div>
                  ) : (
                    <span className="badge-primary text-xs">✓ In Automation</span>
                  )}
                </div>
              </div>
