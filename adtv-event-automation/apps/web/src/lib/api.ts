...
export const apiCampaigns = {
  ...
  sendTest: (id: string, email?: string) => sendJson('POST', `/api/campaigns/${id}/send-test`, email ? { email } : undefined) as Promise<{ ok: boolean; sent: number; total: number; recipient: string; errors?: string[] }>,
};
